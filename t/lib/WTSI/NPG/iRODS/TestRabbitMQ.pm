...
### shared methods for RabbitMQ tests ###

sub rmq_get_subscriber_args {
    my ($channel, ) = @_;
    my $args = {
...
sub rmq_test_collection_message {
    ...
    # 10 tests in total
    return rmq_test_message($message, $method, $expected_body, $irods, 0);
}

sub rmq_test_object_message {
    my ($message, $method, $expected_body, $irods) = @_;
    # 11 tests in total
    return rmq_test_message($message, $method, $expected_body, $irods, 1);
}

sub rmq_test_message {
